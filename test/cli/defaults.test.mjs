...
test('AGENTS.md block documents start gate + load/propose rule + discovery', () => {
  ...
  assert.match(md, /lists each ACTIVE skill/)
  assert.match(md, /moves CORRECTNESS or QUALITY/)
  assert.match(md, /moves COST, SPEED, or response STYLE/)
  assert.match(md, /PROPOSE/)
  assert.match(md, /FORBIDDEN/)
  assert.match(md, /LOADED/)
  assert.match(md, /PRIORITIES/)
  assert.match(md, /correctness > quality > cost/)
